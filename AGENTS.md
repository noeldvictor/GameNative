...
- Cheat/helper overlays must show toggle state directly on the buttons with clear text such as `ON`, `OFF`, `ACTIVE`, or `LOCKED`, plus a distinct active visual state. AYN Thor users should never have to guess whether a cheat press registered.
...
- For the `.hgo` debug package, physical controller input should go raw to `WinHandler` before GameNative profile/remap handling while a game is active. The Android UI can see the Thor controls even when Wine cannot, so prioritize the Android-to-Wine handoff for lab builds. `MainActivity` may route gamepad events directly to the active `XServerView`/`WinHandler` as a hard fallback, with `gncontrol` log evidence.
- AYN Thor controller proof, 2026-05-06: if Odin/GameAssistant key test lights up but the Windows game does not move, first check display/controller focus. On Thor the physical controls can follow the last physically touched screen; tapping the active game screen after launch restored DQH controls. ADB synthetic events are useful to split the bug: `sendevent /dev/input/event9 1 304 1; sendevent /dev/input/event9 0 0 0; sendevent /dev/input/event9 1 304 0; sendevent /dev/input/event9 0 0 0` should produce `gncontrol` logs when the GameNative/Wine bridge is healthy.
- The `.hgo` `libevshim.so` virtual controller updater must reread the shared `gamepad*.mem` state from offset `0` every loop. Use `pread(fd, &cur, sizeof cur, 0)` for regular files; plain `read()` reaches EOF after the first state and makes controls stop updating even though Wine and SDL appear initialized.
